...
print("\nQuantiles solaire (MW) - ARPEGE")
print(df["pv_fr_arpege"].quantile([0.25, 0.5, 0.75]).round(0))

print("\nQuantiles solaires (MW) - STORM")
print(df["pv_fr_storm"].quantile([0.25, 0.5, 0.75]).round(0))

def build_pv_regime(series):
...
df["pv_regime_arpege"] = build_pv_regime(df["pv_fr_arpege"])
df["pv_regime_storm"]  = build_pv_regime(df["pv_fr_storm"])
...
print_regime_stats(df, "pv_regime_arpege", "ARPEGE")
print_regime_stats(df, "pv_regime_storm", "STORM")
...
print_regime_stats_by_season(df, "pv_regime_arpege", "ARPEGE")
print_regime_stats_by_season(df, "pv_regime_storm", "STORM")
...
print_regime_stats_by_daytype(df, "pv_regime_arpege", "ARPEGE")
print_regime_stats_by_daytype(df, "pv_regime_storm", "STORM")
...
print_regime_stats_by_season_daytype(df, "pv_regime_arpege", "ARPEGE")
print_regime_stats_by_season_daytype(df, "pv_regime_storm", "STORM")

# ============================================================
# SCATTER PLOT — FORECAST SOLAIRE STORM VS SPREAD ID-DA
# ============================================================

corr_storm = df["pv_fr_storm"].corr(df["spread_id_da"])
...
plt.title(
    f"Forecast solaire STORM vs Spread ID-DA\n"
    f"Corrélation de Pearson = {corr_storm:.3f}"
)

plt.xlabel("Forecast solaire FR STORM (MW)")
...
# ============================================================
# SCATTER PLOT — FORECAST SOLAIRE ARPEGE VS SPREAD ID-DA
# ============================================================

corr_arpege = df["pv_fr_arpege"].corr(df["spread_id_da"])
...
plt.title(
    f"Forecast solaire ARPEGE vs Spread ID-DA\n"
    f"Corrélation de Pearson = {corr_arpege:.3f}"
)

plt.xlabel("Forecast solaire FR ARPEGE (MW)")
plt.ylabel("Spread ID-DA (€/MWh)")
...
print("\n=== CORRÉLATIONS DE PEARSON — SOLAIRE ET SPREAD ID-DA ===")
